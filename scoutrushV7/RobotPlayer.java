...
public strictfp class RobotPlayer {
    ...
    **/
	...
	public static boolean shotWillHit(MapLocation loc, RobotInfo target) throws GameActionException{
		...
		MapLocation bullet_start_location = loc.add(loc.directionTo(target.getLocation()), (float)1.01);
		...
		for(int i = 0; i < trees.length; i++){
			...
			if(target.getType() == RobotType.SCOUT && bullet_hit_location.distanceTo(trees[i].getLocation()) < 1.02){
				continue;
			}
	...
	public static MapLocation get_best_location() throws GameActionException {
		...
		if(priority_target != null && priority_location.equals(priority_target.getLocation())){
		...
		if(priority_location.x != INVALID_LOCATION.x && rc.getType() != RobotType.ARCHON){
			...
			for(int i = 0; i < num_angles; i++){
				...
				if(i % 2 == 0){
					move_towards_sighting = towards_sighting.rotateLeftRads((int)(i + 1)/2 * 2 * (float)Math.PI / num_angles);
				}
				else{
					move_towards_sighting = towards_sighting.rotateRightRads((int)(i + 1)/2 * 2 * (float)Math.PI / num_angles);
				}
				if(rc.canMove(move_towards_sighting, rc.getType().strideRadius)){
					best_location = rc.getLocation().add(move_towards_sighting, rc.getType().strideRadius);
