        ...
        while (true) {
            ...
            try {
                ...
                if(spots_available >= 1){
                    ...
                    for(int i = 0; i < num_slices; i++){
                        if(rc.canPlantTree(absolute_right.rotateLeftRads(i * 2 * (float)Math.PI/num_slices)) && toggle % 2 == 0){
                            toggle++;
                            rc.plantTree(absolute_right.rotateLeftRads(i * 2 * (float)Math.PI/num_slices));
                        } else if(rc.canBuildRobot(RobotType.SCOUT, absolute_right.rotateLeftRads(i * 2 * (float)Math.PI/num_slices)) && toggle % 4 == 1) {
                            toggle++;
                            rc.buildRobot(RobotType.SCOUT, absolute_right.rotateLeftRads(i * 2 * (float)Math.PI/num_slices));
                        } else if(rc.canBuildRobot(RobotType.SOLDIER, absolute_right.rotateLeftRads(i * 2 * (float)Math.PI/num_slices)) && toggle % 4 == 3) {
                            toggle++;
                            rc.buildRobot(RobotType.SOLDIER, absolute_right.rotateLeftRads(i * 2 * (float) Math.PI / num_slices));
                        }
                    }
                }
        ...
        while (true) {
            ...
            try {
                ...
                if(hasTarget) {
                    if(enemies.length==0) {//we lost it :( keep moving but have limited patience
                        patience++;
                        if(patience > END_OF_PATIENCE) {hasTarget=false; System.out.println("I'm out of patience!");} //give up
                        if(rc.canMove(target_dir, 1)) {
                            rc.move(target_dir, 1);
                        }
                        Clock.yield();
