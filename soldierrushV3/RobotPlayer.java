...
public strictfp class RobotPlayer {
	...
	static MapLocation bugPath_path_start;
    ...
    **/
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        switch (rc.getType()) {
			...
			case SOLDIER: case TANK:
				HeavyInfantry.runHeavyInfantry(rc);
				break;
		}
	}
	 ...
	 * Function moves the robot -- returns nothing.
	...
	public static void bugPathToLoc(MapLocation target){
		try {
			if (RobotPlayer.bugPath_path_destination != target){
				RobotPlayer.bugPath_path_destination = target;
			}
			Direction previous_velocity = RobotPlayer.bugPath_previous_velocity;
			MapLocation m_start = RobotPlayer.bugPath_path_start;
			MapLocation m_end = RobotPlayer.bugPath_path_destination;
			MapLocation closest_point = RobotPlayer.bugPath_closest_point;
			MapLocation previous_location = rc.getLocation();
			if (m_end.equals(rc.getLocation())){
				// We're done bugpathing and don't need to move
			}
			else {
				// If we are on the line as close as the closest point we've been at so far,
				// we attempt to move on the line.
				...
				// If we are not on the line and we are not adjacent to trees, but are adjacent to robots wait a bit.
				// After a bit, start the bugpathing algo over -- most likely we are circling a friendly robot.
				else if (DistanceToLine(previous_location, m_start, m_end) > 0.5
						&& RobotPlayer.robots.length > 0
						&& RobotPlayer.robots[0].getLocation().distanceTo(rc.getLocation()) < 0.2 +RobotPlayer.robots[0].getRadius()+rc.getType().bodyRadius
						&& (RobotPlayer.trees.length == 0
						|| RobotPlayer.trees[0].location.distanceTo(previous_location) > 2 + RobotPlayer.trees[0].getRadius())){
					RobotPlayer.bugPath_patience += 1;
					if (RobotPlayer.bugPath_patience > 10) {
						RobotPlayer.bugPath_path_start = previous_location;
					...
					// Dummy variable
					int i = -1;
					while (!obstacle_found && i < num_angles) {
						i += 1;
					...
					while (!egress_found && i < num_angles) {
						i += 1;
						potential_direction = previous_velocity.rotateRightDegrees((float) 180.0 - (float) 360.0 * i / num_angles);
						if (rc.canMove(potential_direction)) {
				...
				// If we haven't moved, set the path start to where we are because something screwed up
				if (previous_location.equals(current_location)){
					RobotPlayer.bugPath_path_start = current_location;
					RobotPlayer.bugPath_closest_point = current_location;
				}
				// Otherwise, set the previous velocity to the direction we just traveled
				// Plus, check for closest point
				else {
					RobotPlayer.bugPath_previous_velocity = previous_location.directionTo(current_location);
					if (RobotPlayer.DistanceToLine(current_location, m_start, m_end) < 0.5
							&& current_location.distanceTo(m_end) < RobotPlayer.bugPath_closest_point.distanceTo(m_end)) {
						RobotPlayer.bugPath_closest_point = current_location;
					}
				}
			}
	...
	public static MapLocation get_best_location() throws GameActionException {
		...
		if(totalEnemies > 0 && rc.getType() != RobotType.ARCHON){
			...
			if (!best_location.equals(INVALID_LOCATION)) { //CAN MOVE #1
		...
		for(int i = 1; i < last_sighting_location.length; i++) {
			if(!last_sighting_location[i].equals(INVALID_LOCATION)
					//&& rc.canMove(last_sighting_location[i]) CAN MOVE #2
		...
		if(!best_location.equals(INVALID_LOCATION) && rc.getType() != RobotType.ARCHON) { //CAN MOVE #3
