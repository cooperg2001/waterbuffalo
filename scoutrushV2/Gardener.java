        ...
        while (true) {
            ...
            try {
                ...
                if (spots_available >= 1) {
                    ...
                    for (int i = 0; i < RobotPlayer.num_slices; i++) {
                        if (rc.canPlantTree(RobotPlayer.absolute_right.rotateLeftRads(i * 2 * (float) Math.PI / RobotPlayer.num_slices)) && toggle % 2 == 0) {
                            toggle++;
                            rc.plantTree(RobotPlayer.absolute_right.rotateLeftRads(i * 2 * (float) Math.PI / RobotPlayer.num_slices));
                        } else if (rc.canBuildRobot(RobotType.SCOUT, RobotPlayer.absolute_right.rotateLeftRads(i * 2 * (float) Math.PI / RobotPlayer.num_slices)) && toggle % 4 == 1) {
                            toggle++;
                            rc.buildRobot(RobotType.SCOUT, RobotPlayer.absolute_right.rotateLeftRads(i * 2 * (float) Math.PI / RobotPlayer.num_slices));
                        } else if (rc.canBuildRobot(RobotType.SOLDIER, RobotPlayer.absolute_right.rotateLeftRads(i * 2 * (float) Math.PI / RobotPlayer.num_slices)) && toggle % 4 == 3) {
                            toggle++;
                            rc.buildRobot(RobotType.SOLDIER, RobotPlayer.absolute_right.rotateLeftRads(i * 2 * (float) Math.PI / RobotPlayer.num_slices));
                        }
                    }
                }
