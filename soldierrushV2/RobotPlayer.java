    ...
    **/
		...
		try {
			if (m_end == rc.getLocation()){
				...
				if (DistanceToLine(previous_location, m_start, m_end) < 0.5
					...
					if (potential_direction != null && rc.canMove(potential_direction)) {
	...
	public static MapLocation get_best_location() throws GameActionException {
		...
		if(totalEnemies > 0 && rc.getType() != RobotType.ARCHON){
			...
			if (best_location.x != INVALID_LOCATION.x) { //CAN MOVE #1
		...
		if(best_location != INVALID_LOCATION && rc.getType() != RobotType.ARCHON) { //CAN MOVE #3
