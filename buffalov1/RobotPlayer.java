...
public strictfp class RobotPlayer {
	...
	final static int num_angles = 48;
    ...
    **/
	...
	static boolean distFromEdgeAtLeast(float dist) throws GameActionException{
		MapLocation loc = rc.getLocation();
		if(!rc.onTheMap(new MapLocation(loc.x + dist, loc.y))){
			return false;
		}
		if(!rc.onTheMap(new MapLocation(loc.x - dist, loc.y))){
			return false;
		}
		if(!rc.onTheMap(new MapLocation(loc.x, loc.y + dist))){
			return false;
		}
		if(!rc.onTheMap(new MapLocation(loc.x, loc.y - dist))){
			return false;
