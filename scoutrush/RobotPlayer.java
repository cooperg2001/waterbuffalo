...
public strictfp class RobotPlayer {
    ...
    **/
        ...
        while (true) {
            ...
            try {
				...
				if(spots_available > 1){
					...
					for(int i = 0; i < num_slices; i++){
						if(rc.canBuildRobot(RobotType.SCOUT, absolute_right.rotateLeftRads(i * 2 * (float)Math.PI/num_slices))){
							//System.out.println("Building scout...");
							rc.buildRobot(RobotType.SCOUT, absolute_right.rotateLeftRads(i * 2 * (float)Math.PI/num_slices));
						}
					}
	...
	static void runScout() throws GameActionException{
		...
		while(true) {
			...
			try{
				...
				if(enemies.length == 0){
					/*
					//System.out.println("No enemies seen, proceeding to nearest sighting at");
					MapLocation target_loc = getClosestTarget();
					//System.out.println(target_loc.x + " " + target_loc.y);
					if(isValidLoc(target_loc)){
						//System.out.println("Previous sighting valid, proceeding there");
						tryMove(rc.getLocation().directionTo(target_loc));
					}
					else{
						//System.out.println("No previous sightings, moving randomly");
						int trials = 0;
						while(!rc.canMove(rand) && trials < 10){
							rand = randomDirection();
							trials++;
					...
					*/
					...
					/*boolean[] targetable = new boolean[enemies.length];
					for(int i = 0; i < enemies.length; i++){
						targetable[i] = false;
						MapLocation shooting_loc = getBestShootingLocation(enemies[i].getLocation());
						if(isValidLoc(shooting_loc)){
							targetable[i] = true;
						}
					}*/
					int priority = -1;
					for(int i = 0; i < enemies.length; i++){
						...
						else if(enemies[priority].getType() != RobotType.GARDENER && enemies[priority].getType() != RobotType.SOLDIER && enemies[priority].getType() != RobotType.SCOUT && enemies[i].getType() == RobotType.SCOUT){
							priority = i;
						}
					}
					//System.out.println("Going to get best shooting location... " + Clock.getBytecodeNum());
					MapLocation target = getBestShootingLocation(enemies[priority].getLocation());
					if(isValidLoc(target)){
						//System.out.println("Target acquired... " + Clock.getBytecodeNum());
						if(rc.canMove(target)){
							rc.move(target);
							if(rc.canFireSingleShot()){
								rc.fireSingleShot(rc.getLocation().directionTo(enemies[priority].getLocation()));
							}
						}
						else{
							int trials = 0;
	...
	static void checkForStockpile() throws GameActionException{
		try{
			if(rc.getRobotCount() > 100 || rc.getRoundNum() > 2800){
				rc.donate(rc.getTeamBullets() - (rc.getTeamBullets() % 10));
			}
		} catch(Exception e){
			System.out.println("checkForStockpile() error");
	...
	static MapLocation getBestShootingLocation(MapLocation target) throws GameActionException{
		try{
			...
			for(int i = 0; i < trees.length; i++){
				...
				if(rc.getRoundNum() > 100 && rc.getRoundNum() < 140){
					System.out.println("Target is at " + target + " protected by tree at " + trees[i].getLocation() + " for angles " + center_angle + " " + spread);
				}
				for(int j = 0; j < num_slices; j++){
					if(center_angle - spread < potential_angles[j] && potential_angles[j] < center_angle + spread){
    ...
    static void runArchon() throws GameActionException {
        ...
        while (true) {
            ...
            try {
				...
				for(int i = 0; i < num_slices; i++){
					Direction dir = absolute_right.rotateLeftRads(potential_angles[i]);
					if(rc.canHireGardener(dir) && rc.readBroadcast(904) >= 2 * rc.readBroadcast(901) - 3){
						rc.hireGardener(dir);
						break;
					}
				}
	...
	static void runGardener() throws GameActionException {
		...
		rc.broadcast(901, rc.readBroadcast(901));
        ...
        while (true) {
            ...
            try {
				...
				Direction next_build = absolute_right.rotateLeftRads(trees_planted * (float)Math.PI/3).rotateRightRads(2 * (float)Math.PI/3);

				if(trees_planted < 5 && rc.canPlantTree(next_build)){
					rc.plantTree(next_build);
					trees_planted++;
				}
				if(trees_planted == 5 && rc.canBuildRobot(RobotType.SCOUT, absolute_left)){
					rc.buildRobot(RobotType.SCOUT, absolute_left);
				}
	...
	static void findShakableTrees() throws GameActionException{
		try{
			...
			if(trees.length == 0){
				...
				for(int i = 0; i < trees.length; i++){
					if(rc.canShake(trees[i].getLocation())){
						if(trees[i].getContainedBullets() > best.getContainedBullets()){
							best = trees[i];
						}
					}
				}
	...
	static MapLocation getBestShootingLocation(MapLocation target) throws GameActionException{
		try{
			...
			for(int i = 0; i < num_slices; i++){
				...
				Direction potential_dir = absolute_right.rotateLeftRads(potential_angles[i]);
				MapLocation potential_shooting_loc = target.add(potential_dir, getSightRadius());
				if(rc.canSenseLocation(potential_shooting_loc) && rc.canMove(potential_shooting_loc)){
					potential_loc.add(potential_shooting_loc);
				}
			}
